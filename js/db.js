...
const DB_NAME='GymFlowDB',DB_VER=2;let db;

function initDB(){return new Promise((res,rej)=>{const r=indexedDB.open(DB_NAME,3);r.onupgradeneeded=e=>{const d=e.target.result;['plans','workouts','exercises','weights','measurements','goals','settings','timeline'].forEach(s=>{if(!d.objectStoreNames.contains(s))d.createObjectStore(s,{keyPath:'id'})})};r.onsuccess=e=>{db=e.target.result;res(db)};r.onerror=e=>rej(e)})}
